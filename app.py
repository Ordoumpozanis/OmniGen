...
# Fixed model directory path
CUSTOM_MODEL = "../Models/"
# Ensure the specified directory exists
assert Path(CUSTOM_MODEL).exists(), f"Model directory {CUSTOM_MODEL} does not exist."
...
def generate_image(text, img1, img2, img3, height, width, guidance_scale, img_guidance_scale, inference_steps, seed, separate_cfg_infer, offload_model,
                   use_input_image_size_as_output, max_input_image_size, Quantization=False):
    ...
    # Initialize or reuse pipeline with quantization setting
    global pipe
    if pipe is None or pipe.Quantization != Quantization:
        pipe = OmniGenPipeline.from_pretrained(CUSTOM_MODEL, Quantization=Quantization)
